...
// succeeded.
func (tr *TestRunner) ProxyRadiusAndAssertSuccess(imsi string) {
	radiusP, err := tr.ProxyRadius(imsi, defaultCalledStationID)
	assert.NoError(tr.t, err)
	assert.True(tr.t, (radiusP.Code == radius.CodeAccessAccept))
}
